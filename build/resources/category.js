...
export async function getSatellitesCategoryResource(apiClient, uri) {
    try {
        // Extract the category ID from the URI
        const match = uri.match(/^satellites:\/\/category\/(\d+)$/);
        if (!match) {
            throw new McpError(ErrorCode.InvalidRequest, `Invalid satellites category resource URI: ${uri}`);
        }
        const categoryId = parseInt(match[1], 10);
        // Validate category ID
        if (!Number.isInteger(categoryId) || categoryId < 0 || categoryId > 30) {
            throw new McpError(ErrorCode.InvalidParams, 'Invalid category ID. Must be an integer between 0 and 30.');
        }
        ...
        throw new McpError(ErrorCode.InternalError, `Error retrieving satellites by category: ${error.message}`);
    }
}
