...
export async function getSatellitesAboveResource(apiClient, uri) {
    try {
        // Extract the parameters from the URI
        const match = uri.match(/^satellites:\/\/above\/([^/]+)\/([^/]+)\/([^/]+)$/);
        if (!match) {
            throw new McpError(ErrorCode.InvalidRequest, `Invalid satellites above resource URI: ${uri}`);
        }
        ...
        if (isNaN(latitude) || latitude < -90 || latitude > 90) {
            throw new McpError(ErrorCode.InvalidParams, 'Invalid latitude. Must be a number between -90 and 90.');
        }
        if (isNaN(longitude) || longitude < -180 || longitude > 180) {
            throw new McpError(ErrorCode.InvalidParams, 'Invalid longitude. Must be a number between -180 and 180.');
        }
        // Validate radius
        if (isNaN(radius) || radius < 0 || radius > 90) {
            throw new McpError(ErrorCode.InvalidParams, 'Invalid radius. Must be a number between 0 and 90 degrees.');
        }
        const params = {
        ...
        throw new McpError(ErrorCode.InternalError, `Error retrieving satellites above location: ${error.message}`);
    }
}
