...
export async function getSatelliteResource(apiClient, uri) {
    try {
        // Extract the NORAD ID from the URI
        const match = uri.match(/^satellite:\/\/(\d+)$/);
        if (!match) {
            throw new McpError(ErrorCode.InvalidRequest, `Invalid satellite resource URI: ${uri}`);
        }
        ...
        if (!tleData && !positions) {
            throw new McpError(ErrorCode.InvalidRequest, `No data found for satellite with NORAD ID: ${noradId}`);
        }
        ...
        throw new McpError(ErrorCode.InternalError, `Error retrieving satellite resource: ${error.message}`);
    }
}
