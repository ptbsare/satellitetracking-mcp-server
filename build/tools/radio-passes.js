...
export async function predictRadioPassesTool(apiClient, args) {
    try {
        ...
        if (days !== undefined && (!Number.isInteger(days) || days < 1 || days > 10)) {
            throw new McpError(ErrorCode.InvalidParams, 'Invalid days. Must be an integer between 1 and 10.');
        }
        // Validate min_elevation
        if (min_elevation !== undefined && (min_elevation < 0 || min_elevation > 90)) {
            throw new McpError(ErrorCode.InvalidParams, 'Invalid min_elevation. Must be between 0 and 90 degrees.');
        }
        const params = {
            noradId: norad_id,
            observer_lat,
        ...
        throw new McpError(ErrorCode.InternalError, `Error predicting satellite radio passes: ${error.message}`);
    }
}
...
//# sourceMappingURL=radio-passes.js.map