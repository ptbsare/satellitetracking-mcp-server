...
export async function getSatellitePositionTool(apiClient, args) {
    try {
        const { norad_id, observer_lat, observer_lng, observer_alt, seconds } = args;
        // Validate NORAD ID
        if (!Number.isInteger(norad_id) || norad_id <= 0) {
            throw new McpError(ErrorCode.InvalidParams, 'Invalid NORAD ID. Must be a positive integer.');
        }
        // Validate coordinates
        if (observer_lat < -90 || observer_lat > 90) {
            throw new McpError(ErrorCode.InvalidParams, 'Invalid latitude. Must be between -90 and 90.');
        }
        if (observer_lng < -180 || observer_lng > 180) {
            throw new McpError(ErrorCode.InvalidParams, 'Invalid longitude. Must be between -180 and 180.');
        }
        // Validate seconds
        if (seconds !== undefined && (seconds < 1 || seconds > 300)) {
            throw new McpError(ErrorCode.InvalidParams, 'Invalid seconds. Must be between 1 and 300.');
        }
        const params = {
            noradId: norad_id,
            observer_lat,
        ...
        if (!positions || positions.length === 0) {
            throw new McpError(ErrorCode.InvalidRequest, `No position data found for satellite with NORAD ID: ${norad_id}`);
        }
        // Format the response
        ...
        throw new McpError(ErrorCode.InternalError, `Error retrieving satellite position: ${error.message}`);
    }
}
...
//# sourceMappingURL=satellite-position.js.map