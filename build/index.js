...
class SatelliteTrackingServer {
    ...
    getApiClient() {
        if (!this.apiClient) {
            ...
            if (!apiKey) {
                throw new McpError(ErrorCode.InvalidRequest, 'N2YO_API_KEY environment variable is required');
            }
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            ...
            switch (request.params.name) {
                case 'get_satellite_position':
                    return getSatellitePositionTool(apiClient, request.params.arguments);
                case 'get_satellite_tle':
                    return getSatelliteTLETool(apiClient, request.params.arguments);
                case 'predict_visual_passes':
                    return predictVisualPassesTool(apiClient, request.params.arguments);
                case 'predict_radio_passes':
                    return predictRadioPassesTool(apiClient, request.params.arguments);
                case 'get_satellites_above':
                    return getSatellitesAboveTool(apiClient, request.params.arguments);
                case 'search_satellites_by_name':
                    return searchSatellitesByNameTool(apiClient, request.params.arguments);
                case 'search_satellites_by_category':
                    return searchSatellitesByCategoryTool(apiClient, request.params.arguments);
                default:
